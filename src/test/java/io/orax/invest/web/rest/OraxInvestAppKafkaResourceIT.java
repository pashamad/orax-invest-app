package io.orax.invest.web.rest;
...
import io.orax.invest.config.KafkaProperties;
...
class OraxInvestAppKafkaResourceIT {
    ...
    void setup() {
        ...
        OraxInvestAppKafkaResource kafkaResource = new OraxInvestAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/orax-invest-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/orax-invest-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
